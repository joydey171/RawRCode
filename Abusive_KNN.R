...
#setwd("E:/Zreyas/Abusive/Algo_1.2/")
...
n_train <- nrow(train)
rel_col_trn <- c("p_stat","kurtosis","sd","crest","X95p_T","mn_T","label")
train <- train[,rel_col_trn]

test$p_stat <- as.factor(ifelse(test$v/test$b > 5,"H","L"))
rel_col_tst <- c("p_stat","kurtosis","sd","crest","X95p_T","mn_T")
...
set.seed(330)
library(kknn)
mod <- kknn(label~.,train,test_dum,kernel = "optimal")
